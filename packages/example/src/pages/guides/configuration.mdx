...
<AnchorLinks>
  ...
  <AnchorLink>Header nav</AnchorLink>
...
The default header for the theme is the left nav sidebar. However, we have added a configuration option that allows you to switch from the default left nav to a header nav.

We recommend using a header nav for sites that might not have many pages or tabs. If in the future, the content on your site evolves and you feel the left nav is better suited, you can always change it back.

To enable the header nav, provide `header` as a string to the `navigationStyle` option. To switch back to the default left nav provide an empty string `''` to the `navigationStyle` option.
...
```
When enabled, your header nav will look like the image below: 

<Row>
<Column colMd={8} colLg={8}>

![Header nav](header-nav-config.png)

</Column>
</Row>

With the header nav, the items on your page will be further left-aligned to allow for more content space.

**Just a note:** In mobile, when header nav is enabled, the left nav will persist.
## Edit on GitHub link
