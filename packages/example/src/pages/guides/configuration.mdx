...
To add a link to the bottom of each page that points to the current page source in GitHub, provide a `repository` object to `siteMetadata` in your `gatsby-config.js` file. You can provide a `url`, and if needed, the `subDirectory` and `branch` where your site source lives.
