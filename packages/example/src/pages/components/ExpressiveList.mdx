...
The `<ExpressiveList>` component is meant to be used with the `expressive-content-list-group` className when grouped together. This component is only intended for paragraph content and not meant to nest other components within it. `ExpressiveListContainer` coming soon. This can be used with a pictogram by importing a pictogram from our library and passing it into the `picogram` prop as a node.
