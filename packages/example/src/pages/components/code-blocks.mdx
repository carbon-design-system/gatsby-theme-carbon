...
```markdown path=/directory/file.mdx copyLineStart=2
### Copy from line 2 (0-offset)

this line will be copied
all the way to the end (this one too)
```

```markdown path=/directory/file.mdx copyLineStart=2 copyLineLength=1
### Copy from line 2 of length 1

this line will be copied
but not this one
```

```markdown path=/directory/file.mdx copyLineLength=1
### Copy of length one (from start)

only the header will be copied
```
