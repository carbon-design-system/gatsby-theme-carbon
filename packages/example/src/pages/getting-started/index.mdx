...
This project is a [Gatsby theme](https://www.gatsbyjs.org/docs/themes/what-are-gatsby-themes/) that's meant to empower developers, content writers, and designers to create a fast, accessible site that looks and feels like Carbon. Follow along to get started with creating your own site.
...
Navigate into your directory with `cd my-carbon-site`, then start it up using one of the following snippets. You can tell which command to use based on the lock file at the root of your project (`yarn.lock` for yarn and `package-lock.json` for npm).

For yarn, type `yarn develop` for npm, you'll use `npm run develop`.

3. **Make some changes!**

Navigate to [localhost:8000](//localhost:8000) to see your site running
