...
import { match, keys } from 'carbon-components-react/lib/tools/key';
import { 
  gtc__title,
  gtc__content,
  gtc__heading,
...
export default class AccordionItem extends Component {
  ...
  handleKeyDown = evt => {
    if (
      ...
      this.state.open &&
      evt.target.classList.contains(`${prefix}--accordion__heading ${gtc__heading}`)
    ) {
  ...
  render() {
    ...
    const classNames = classnames(
      {
        [`${prefix}--accordion__item--active ${gtc__item__active}`]: this.state.open,
      },
      `${prefix}--accordion__item ${gtc__item}`,
      className
    );
    return (
      <li
        ...
        <Expando
          ...
          className={`${prefix}--accordion__heading ${gtc__heading}`}
          ...
          />
          <div className={`${prefix}--accordion__title ${gtc__title}`}>{title}</div>
        </Expando>
        <div className={`${prefix}--accordion__content ${gtc__content}`}>{children}</div>
      </li>
    );
  }
}